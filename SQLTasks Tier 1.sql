...
Code:
SELECT DISTINCT CONCAT(m.firstname, ' ', m.surname) AS mem_name
FROM Bookings AS b
...
Code:
SELECT CONCAT(m.firstname, ' ', m.surname) AS person, f.name AS facname,
...
Code:
SELECT CONCAT(m.firstname, ' ', m.surname) AS person, 
       f.name AS facname, 
       CASE WHEN b.memid = 0 THEN b.slots * f.guestcost 
